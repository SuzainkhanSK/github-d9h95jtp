  ...
  const handleClaimReward = async (taskId: string) => {
    ...
    try {
      ...
      if (isSupabaseConfigured && user?.id) {
        // Use atomic function to process special ad task
        const { data: taskResult, error: taskError } = await supabase.rpc('process_special_ad_task', {
          user_id_param: user.id,
          task_id_param: taskId,
          task_title_param: adTasks[taskIndex].title
        });
